...
// Model for Contact entity
    models.Contact = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/contacts-jaxrs/webresources/contact/",
        idAttribute: 'id',
        defaults: {
            phone: "",
            name: "",
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/contacts-jaxrs/webresources/contact/';
            }
    ...
    // Collection class for Contact entities
    models.ContactCollection = Backbone.Collection.extend({
        model: models.Contact,
        url: "http://localhost:8080/contacts-jaxrs/webresources/contact/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
            ...
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        ...
                        // tpl-contact-list-itemis template identifier for item
                        templateName: '#tpl-contact-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.contact = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.contact,
                    // tpl-contact-details is template identifier for chosen model element
                    templateName: '#tpl-contact-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Contact();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-contact-details is a template identifier for chosen model element
                templateName: '#tpl-contact-details',
